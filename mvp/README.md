# shortener · MVP (без Redis и LB)

Простой вариант: один инстанс backend, кэш и rate limiter — in-process, без отдельных сервисов.
Общее ФТ/НФТ и сравнение с HA-вариантом — в [корневом README](../README.md).
...
Кэш и rate limiter — это компоненты внутри процесса backend (не отдельные сервисы и не Redis), поэтому состояние не переживает перезапуск и не шарится между несколькими инстансами backend — это осознанное упрощение для MVP. Вариант с общим состоянием между инстансами — [ha/](../ha/).
...
│   └── Dockerfile
├── deploy/yandex-cloud/cloud-init.yaml
├── docker-compose.yml
├── DEPLOY.md
└── .env.example
```

Общая OpenAPI-спецификация — на уровень выше, в [../api/links.json](../api/links.json).
...
Инструкция по деплою на одну ВМ в Яндекс.Облаке (Compute Cloud) — в [DEPLOY.md](DEPLOY.md). Также можно развернуть через [Coolify](https://coolify.io/) как отдельное Docker Compose-приложение с Base directory `mvp`.
...
| `FRONTEND_PORT`                    | `3000`       | Порт, на котором frontend слушает на хосте     |
