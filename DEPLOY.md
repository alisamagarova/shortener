...
наружу не открываются — в `docker-compose.yml` они вообще не публикуются на хост,
сервисы общаются между собой через внутреннюю docker-сеть по именам (`db`, `backend`).
